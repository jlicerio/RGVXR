/**
 * canvas-setup — Initializes the A-Frame canvas for HTML-in-Canvas API.
 * Must be attached to <a-scene> to enable layoutsubtree for spatial UI.
 */
AFRAME.registerComponent('canvas-setup', {
  init: function() {
    var el = this.el;
    el.addEventListener('loaded', function() {
      ...
      // Add layoutsubtree for HTML-in-Canvas spatial panels
      if (window.HTMLInCanvasBridge && window.HTMLInCanvasBridge.supported) {
        canvas.setAttribute('layoutsubtree', '');
        console.log('[canvas-setup] layoutsubtree enabled on canvas');
      }
    });
  }
});
...
// WebXR/HTML-in-Canvas spatial UI helpers.
// showToast creates a brief notification. If HTML-in-Canvas bridge is active,
// it renders as a canvas child; otherwise falls back to body append.
function showToast(message) {
  var bridge = window.HTMLInCanvasBridge;
  var canvas = document.querySelector('a-scene') ? document.querySelector('a-scene').canvas : null;
  
  // Try canvas-based toast via HTML-in-Canvas
  if (bridge && bridge.active && canvas) {
